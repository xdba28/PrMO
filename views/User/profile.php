		...
		$success_notifs=[];
		 ...
		 if(Token::check("nametoken", Input::get('nametoken'))){
			 try{
				...
				$user->startTrans();
					$user->update("enduser", "edr_id", Input::get("user"), array(
						'edr_fname' => Input::get("fname"),
						'edr_mname' => Input::get("mname"),
						'edr_lname' => Input::get("lname"),
						'edr_ext_name' => $extname
					));
			...
			try{
				...
				$user->startTrans();
					$user->update("enduser", "edr_id", Input::get("user"), array(
						'edr_designated_office' => Input::get("newunit")
					));
			...
			if($validation->passed()){
				try{
					...
					$user->startTrans();
						$user->update("edr_account", "account_id", Input::get("user"), array(
					...
					// Check if image file is a actual image or fake image
						...
						if ($target_file == "../data/profile_images/") {							
							...
							if (move_uploaded_file($_FILES["profilePhoto"]["tmp_name"], $target_file)) {
								...
								$user->startTrans();
									$user->update("enduser", "edr_id", Session::get(Config::get('session/session_name')), array(
										'edr_profile_photo' => $new_filename
									));
			...
			<div class="row m-b-lg m-t-lg">
				...
				<a href="#" data-toggle="modal" data-target="#profile-photo-modal"><span class="label label-warning" style="position: absolute;display: inline-block;height: 0;line-height: 0; border: 0.6em solid transparent;left: 2.8em;text-align: left;top: 6em;bottom: 0em;">Edit
					...
					<div class="profile-info">
							<?php 
								...
								$accountData = $user->get("edr_account", array("account_id", "=", Session::get(Config::get("session/session_name"))));
								$userDesignation =  $user->get("units", array("ID", "=", $userData->edr_designated_office));
				...
				<div class="col-md-3">
					<table class="table small m-b-xs">
						...
						<tr>
							<td>
								<strong style="color:black"><?php 
								$reports = $user->dashboardReports($user->data()->account_id);
								
								echo $reports->projectStatusCount->TOTAL_PROJECTS;
								?></strong> Projects
							</td>
							<td>
								<strong><?php echo $reports->request_forms_related->REQUEST_FORMS;?></strong> Request Forms Created
							</td>

						</tr>
						<tr>
							<td>
								<strong style="color:#28a745"><?php echo $reports->projectStatusCount->ONGOING;?></strong> Ongoing
							</td>
							<td>
								<strong><?php echo $reports->request_forms_related->UNRECEIVED;?></strong> Uncreceived
							</td>
						</tr>
						<tr>
							<td>
								<strong style="color:#007bff"><?php echo $reports->projectStatusCount->FINISHED;?></strong> Finished
							</td>
							<td>
								<strong><?php echo $reports->request_forms_related->REVISION_REQUEST;?></strong> Revision Requests
							</td>
						</tr>
						<tr>
							<td>
								<strong style="color:red"><?php echo $reports->projectStatusCount->FAILED;?></strong> Failed
							</td>
						</tr>						
						</tbody>
					</table>
                            ...
                            <div id="tab-1" class="tab-pane active show">
                                <div class="panel-body">
                                    ...
                                    <div class="form-group row" style="margin-bottom:0px">
										...
										<div class="col-lg-2"><button class="btn btn-default btn-xs" type="button" style="margin-top:5px" data-toggle="modal" data-target="#profile-setting" data-rcontent="name" data-user="<?php echo $user->data()->account_id;?>" data-fname="<?php echo $userData->edr_fname;?>" data-mname="<?php echo $userData->edr_mname;?>" data-lname="<?php echo $userData->edr_lname;?>" data-extname="<?php echo $userData->edr_ext_name;?>" ><i class="fas fa-edit"></i> Edit</button></div>
									</div>
									<div class="hr-line-dashed" style="margin:0px 0"></div>		
									
                                    <div class="form-group row" style="margin-bottom:0px">
										<label class="col-lg-3 col-form-label" style="font-weight:bold">Email</label>
										<div class="col-lg-7" style="margin-top:5px"><p class="form-control-static" ><?php echo $userData->edr_email;?></p></div>
										<div class="col-lg-2"><button class="btn btn-default btn-xs" type="button" style="margin-top:5px" data-toggle="modal" data-target="#profile-setting-small" data-user="<?php echo $user->data()->account_id;?>" data-rcontent="email" data-toupdate="<?php echo $userData->edr_email;?>"><i class="fas fa-edit"></i> Edit</button></div>
									</div>
									<div class="hr-line-dashed" style="margin:0px 0"></div>		
